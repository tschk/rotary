...
impl IpcServer {
    ...
    async fn execute_method(&self, method: &str, params: &Value) -> Result<Value, String> {
        match method {
            ...
            "state" => self.execute_state().await,
            "tools" => self.execute_tools(),
            "plugins" => self.execute_plugins(),
            "messages" => self.execute_messages().await,
            "set_model" => self.execute_set_model(params).await,
            "set_scope" => self.execute_set_scope(params).await,
            "get_policy" => self.execute_get_policy().await,
            "set_policy" => self.execute_set_policy(params).await,
            "set_approver" => self.execute_set_approver(params).await,
            "clear_authorizer" => self.execute_clear_authorizer().await,
            "prompt" => self.execute_prompt(params).await,
            "session_list" => self.execute_session_list(),
            "session_clear" => self.execute_session_clear().await,
            _ => Err(format!("unknown method: {method}")),
        }
    }

    async fn execute_state(&self) -> Result<Value, String> {
    ...
    fn execute_tools(&self) -> Result<Value, String> {
        Ok(Value::Array(
            self.tools
                .lock()
                .unwrap_or_else(|e| e.into_inner())
    ...
    fn execute_plugins(&self) -> Result<Value, String> {
        #[cfg(feature = "marketplace")]
        {
    ...
    async fn execute_messages(&self) -> Result<Value, String> {
        let agent = self.agent.lock().await;
    ...
    async fn execute_set_model(&self, params: &Value) -> Result<Value, String> {
        let model = params
            .get("model")
            .and_then(|m| m.as_str())
            .unwrap_or("gpt-4o");
    ...
    async fn execute_set_scope(&self, params: &Value) -> Result<Value, String> {
    ...
    async fn execute_get_policy(&self) -> Result<Value, String> {
        let agent = self.agent.lock().await;
        serde_json::to_value(&agent.policy).map_err(|e| e.to_string())
    }

    async fn execute_set_policy(&self, params: &Value) -> Result<Value, String> {
    ...
    async fn execute_set_approver(&self, params: &Value) -> Result<Value, String> {
    ...
    async fn execute_clear_authorizer(&self) -> Result<Value, String> {
        self.agent.lock().await.clear_authorizer();
        Ok(Value::String("authorizer cleared".into()))
    }

    async fn execute_prompt(&self, params: &Value) -> Result<Value, String> {
        let text = params
            .get("text")
            .and_then(|t| t.as_str())
            .unwrap_or("")
    ...
    fn execute_session_list(&self) -> Result<Value, String> {
        let s = self.session.lock().unwrap_or_else(|e| e.into_inner());
        Ok(serde_json::json!({"id": s.id, "entries": s.entries.len()}))
    }

    async fn execute_session_clear(&self) -> Result<Value, String> {
        self.session
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .entries
