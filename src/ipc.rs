        ...
        match method {
            ...
            "state" => {
                ...
                Ok(serde_json::json!({
                    ...
                    "tools": self.tools.lock().map_err(|_| "tools mutex poisoned")?.count(),
                    "plugins": self.plugins.lock().map_err(|_| "plugins mutex poisoned")?.count(),
                }))
            }
            "tools" => Ok(Value::Array(self.tools.lock().map_err(|_| "tools mutex poisoned")?.definitions())),
            "plugins" => {
                let p = self.plugins.lock().map_err(|_| "plugins mutex poisoned")?;
                Ok(Value::Array(
                    p.plugins
                        .iter()
            ...
            "session_list" => {
                let s = self.session.lock().map_err(|_| "session mutex poisoned")?;
                Ok(serde_json::json!({"id": s.id, "entries": s.entries.len()}))
            }
            "session_clear" => {
                self.session.lock().map_err(|_| "session mutex poisoned")?.entries.clear();
